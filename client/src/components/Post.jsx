...
import {dateYearAgo} from "../lib/date.js";
    ...
    return (
        <article
            ...
            <div className="flex justify-between items-center mb-5 text-gray-500">
                ...
                <span className="text-sm">{dateYearAgo(post?.createdAt)}</span>
            </div>
            <h2 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
                <Link to={`/posts/${post?.id}`}>{post?.title}</Link>
            </h2>
            <p className="mb-5 font-light text-gray-500 dark:text-gray-400" dangerouslySetInnerHTML={
